...
namespace NConsoler
{
	...
	public sealed class Consolery
	{
		...
		private MethodInfo GetCurrentMethod()
		{
			...
			return GetMethodByName(_args[0].ToLowerInvariant());
		}
		...
		private MethodInfo GetMethodByName(string name)
		{
			...
			{
				if (method.Name.ToLowerInvariant() == name)
				{
					return method;
				}
			}
		...
		{
			string subcommand = IsMulticommand ? method.Name.ToLowerInvariant() + " " : String.Empty;
		...
		private string ProgramName()
		{
			...
			{
				return _targetType.Name.ToLowerInvariant();
			}
		...
		{
			MethodInfo method = GetMethodByName(_args[1].ToLowerInvariant());
			if (method == null)
			{
				PrintGeneralMulticommandUsage();
				throw new NConsolerException("Unknown subcommand \"{0}\"", _args[0].ToLowerInvariant());
			}
			PrintUsage(method);
		}
		...
		private void PrintGeneralMulticommandUsage()
		{
			...
			foreach (MethodInfo method in _actionMethods)
			{
				_messenger.Write(method.Name.ToLowerInvariant() + " " + GetMethodDescription(method));
			}
		}
		...
		private void CheckActionMethodNamesAreNotReserved()
		{
			foreach (MethodInfo method in _actionMethods)
			{
				if (method.Name.ToLowerInvariant() == "help")
				{
